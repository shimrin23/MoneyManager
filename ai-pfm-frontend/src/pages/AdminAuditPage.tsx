...
const ACTION_ICONS: Record<string, string> = {
    LOGIN: '🔑', USER_DISABLED: '🚫', USER_ENABLED: '✅', CONFIG_CHANGED: '⚙️',
    ...
    return (
        <div className="page-container">
            ...
            <div className="rec-summary-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)' }}>
                ...
                <div className="rec-stat-card" style={{ borderColor: 'rgba(239,68,68,0.3)' }}><div className="rec-stat-icon">🚨</div><div><div className="rec-stat-value" style={{ color: '#ef4444' }}>{logs.filter(l => l.severity === 'critical').length}</div><div className="rec-stat-label">Critical</div></div></div>
                <div className="rec-stat-card pending"><div className="rec-stat-icon">⚠️</div><div><div className="rec-stat-value" style={{ color: '#f59e0b' }}>{logs.filter(l => l.severity === 'warning').length}</div><div className="rec-stat-label">Warnings</div></div></div>
                <div className="rec-stat-card accepted"><div className="rec-stat-icon">ℹ️</div><div><div className="rec-stat-value">{logs.filter(l => l.severity === 'info').length}</div><div className="rec-stat-label">Info</div></div></div>
            </div>
